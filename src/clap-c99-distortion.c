...
uint32_t c99dist_latency_get(const clap_plugin_t *plugin)
{
    return 0;
}
...
bool c99dist_param_get_info(const clap_plugin_t *plugin, uint32_t param_index, clap_param_info_t *param_info)
{
    switch(param_index)
    {
    case 0: // drive
...
bool c99dist_param_value_to_text(
    const clap_plugin_t *plugin, clap_id param_id, double value, char *display, uint32_t size)
{
    ...
    case pid_MIX:
        ...
        break;
    case pid_MODE:
        snprintf(display, size, "%d", (int)value);
        return true;
        break;
    }
    return false;
}
bool c99dist_text_to_value(const clap_plugin_t *plugin,
                      clap_id              param_id,
                      const char          *display,
                      double              *value)
{
...
void c99dist_flush(const clap_plugin_t        *plugin,
...
static const clap_plugin_params_t s_c99dist_params = {
    .count = c99dist_param_count,
